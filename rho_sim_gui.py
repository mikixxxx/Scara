...
def choose_ik_solution(x, y, previous=None):
    ...
    if previous is None:
        # Prefer negative A2 to match our current test convention.
        return min(solutions, key=lambda q: (q[1] >= 0.0, abs(q[0])))
...
class SimulatorView:
    ...
    def update_view(self):
        ...
        if ik is None:
            ik_text = "A1=---      A2=---      XY: UNREACHABLE"
        else:
            ik_text = f"A1={ik[0]:8.2f}°  A2={ik[1]:8.2f}°"
        ...
        self.info.config(
            text=(
                ...
                f"SPEED={speed*100.0:.2f}%  "
                f"AUTO={int(auto)} ALARM={int(alm)} REF={int(ref)}\n"
                f"A1 limit check: {'ON' if CHECK_A1_LIMIT else 'OFF (waiting for calibration)'}"
            )
        )
